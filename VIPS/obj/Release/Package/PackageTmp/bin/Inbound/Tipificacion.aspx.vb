...
Public Class Inbound
    ...
    Sub ComboTiendasQuejas()
        ...
        If Not IsPostBack Then
            ...
            DropDownList14.Items.Add(New ListItem("-Selecciona-", ""))
            DropDownList14.AppendDataBoundItems = True
            ...
            DropDownList14.DataTextField = "Tienda"
    ...
    Sub GetUltimoPedido()
        ...
        Else
            ...
            StatusDelivery(ds.Tables(0).Rows(0).Item(2).ToString)
    ...
    Public Sub GetCustomer(Tel As String)
        ...
        ElseIf ds.Tables(0).Rows.Count = 1 Then
            ...
            Cliente.Value = ds.Tables(0).Rows(0).Item(0).ToString
    ...
    Sub GuardarQuejaNueva()
        ...
        If DropDownList14.SelectedItem.Text = "-Selecciona-" Then
            restaurante = "NULL"
        Else
            restaurante = "'" & DropDownList14.SelectedItem.Text & "'"
        End If
        ...
        idMitrol = Session("RVT")
        id_ccs = x.GetUserUsername(Session("RVT"))
    ...
    Sub UpdateQueja()
        ...
        If DropDownList13.SelectedItem.Value = 0 Then
            ...
            Dim strConnString As String = ConfigurationManager.ConnectionStrings("VIPS").ConnectionString
            Dim strQuery As String = "EXEC [dbo].[CLOSE_Queja] @ID_QUEJA = '" & Session("QuejaID") & "',@DESCRIPCION = '" & TextBox49.Text & "',@ID_CCS = '" & x.GetUserUsername(Session("RVT")) & "',@ID_MITROL = '" & Session("RVT") & "'"
            Dim con As New SqlConnection(strConnString)
            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.Text
            cmd.CommandText = strQuery
        ...
        Else
            ...
            Dim strConnString As String = ConfigurationManager.ConnectionStrings("VIPS").ConnectionString
            Dim strQuery As String = "EXEC [dbo].[UPDATE_Queja] @ID_QUEJA = '" & Session("QuejaID") & "',@DESCRIPCION = '" & TextBox49.Text & "',@ID_CCS = '" & x.GetUserUsername(Session("RVT")) & "',@ID_MITROL = '" & Session("RVT") & "'"
            Dim con As New SqlConnection(strConnString)
            Dim cmd As New SqlCommand()
            cmd.CommandType = CommandType.Text
            cmd.CommandText = strQuery
    ...
    Sub LoadGeneralQuejas()
        ...
        Dim da As New System.Data.SqlClient.SqlDataAdapter
        Dim ds As New System.Data.DataSet
        Dim cmd As SqlCommand = New SqlCommand("SELECT ISNULL(restaurante,''),ISNULL(repartidor,''),ISNULL(pedido,''),DATEPART(DAY,GETDATE()-[fecha_alta]) as dias FROM CRM_VIPS.dbo.SYS_Quejas WHERE no_queja = '" & Session("QuejaID") & "'", conexion)
    ...
    Sub UpdatePedido()
        ...
        'Dim strQuery As String = "EXEC [dbo].[UPADTE_Pedido_CCS] @ID = '" & y.GetPedidoCS(TextBox26.Text) & "',@ID_CLIENTE = '" & id_cliente & "',@NO_DRIVE = '" & no_drive & "',@ID_CCS = '" & Session("RVT") & "',@ID_LLAMADA = '" & Session("ID_MITROL") & "', @MEDIO = '1'"
        'Dim con As New SqlConnection(strConnString)
        'Dim cmd As New SqlCommand()
    ...
    Sub GetDetallesDrive(ID As String)
        ...
        Catch ex As Exception
            ...
            msgtipo(0) = 4
            msgmensaje(0) = "¡Ningun repartidor ha aceptado el pedido!"
            Alerta.NewShowAlert(msgtipo, msgmensaje, Me)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("ani") = "5550831300" Then
        Else
            TextBox5.Text = Session("ani")
        End If
        ...
        If Not IsPostBack Then
            ...
            If Session("campania") = "VIPS_PEDIDO" Then
                Label75.Text = "Nuevo Pedido"
                Label75.ForeColor = System.Drawing.ColorTranslator.FromHtml("#43BD00")
            ElseIf Session("campania") = "VIPS_SEGUIMIENTO" Then
                Label75.Text = "Seguimiento a Pedido"
                Label75.ForeColor = System.Drawing.ColorTranslator.FromHtml("#FF5A01")
                Label8.Visible = True
                ...
                DropDownList5.SelectedValue = 3
                x.LlenaTipificaciones(DropDownList5, DropDownList6, 3)
                ConsultaPedidos.Visible = True
                mapContainer.Visible = True
                LoadPedidosActivos(2)
            ElseIf Session("campania") = "VIPS_ATC" Then
                Label75.Text = "Quejas"
                Label75.ForeColor = System.Drawing.ColorTranslator.FromHtml("#FE0000")
            Else
                Label75.Text = "[SKILL NO RECONOCIDO]"
            End If
        End If
    ...
    Private Sub GridView2_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles GridView2.RowCommand
        If e.CommandName = "Select" Then
            ...
            Cliente.Value = String.Format("{1}", e.CommandArgument, GridView2.Rows(Convert.ToInt32(e.CommandArgument)).Cells(0).Text)
            GetCustomerForID(Session("CustomerID"))
            UltimaInteraccion()
            GetUltimaQueja()
            GetUltimoPedido()
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If DropDownList5.SelectedItem.Text = "Cancelacion" Or DropDownList5.SelectedItem.Text = "Retribucion" Then
            ...
            'MensajeNoVenta = "Tienda: " & DropDownList10.SelectedItem.Value & " </br> Nombre Tienda: " & NombreTienda(DropDownList10.SelectedItem.Value) & "</br> Cliente: " & TextBox1.Text & " " & TextBox2.Text & " " & TextBox4.Text & " </br> Telefono: " & TextBox5.Text & " </br> Domicilio: " & TextBox8.Text & " " & TextBox9.Text & " " & TextBox10.Text & " </br> Fecha: " & Format(Today(), "dd/MM/yyyy") & " </br> Monto: $" & TextBox32.Text
            ...
            If HiddenField7.Value = 1 Then
                ...
                If DropDownList5.SelectedItem.Text = "Nuevo Pedido" And Session("CustomerID") = Nothing And CheckBox1.Checked = False Then

                    Dim script1 As String = "document.getElementById('miError').click();"
                    ScriptManager.RegisterStartupScript(Me, GetType(Page), "modalError", script1, True)
                    ...
                    If DropDownList5.Visible = True Then
                        ...
                        If DropDownList5.SelectedItem.Text = "Nueva Queja" Then
                            ...
                            If GetMailingIndex(GetLastLevel(), GetLastLevelDD()) <> 0 Then
                                If DropDownList5.SelectedItem.Text = "Nueva Queja" Then
                                    If DropDownList14.Visible = False Then
                                        LD = GetCorreo(DropDownList10.SelectedItem.Value)
                                    Else
                                        LD = GetCorreo(DropDownList10.SelectedItem.Value)
                                    End If
                        ...
                        ElseIf DropDownList5.SelectedItem.Text = "Nuevo Pedido" Then
                            EnviarPedido()
                            UpdatePedido()
                            ClientScript.RegisterStartupScript(GetType(Page), "cualquiercosa", "alv();", True)
                        ...
                        ElseIf (DropDownList5.SelectedItem.Text = "Cancelacion" Or DropDownList5.SelectedItem.Text = "Retribucion") And HiddenField5.Value = 1 And HiddenField7.Value = 1 Then
                            ...
                            If DropDownList5.SelectedItem.Text = "Cancelacion" Then
                                Alerta.EnviarMail2(GetCorreo(DropDownList10.SelectedItem.Value), "enrique.escalante@alsea.com.mx,roni.cirilo@alsea.com.mx, raul.ayala@ccscontactcenter.com, nancy.souberbielle@ccscontactcenter.com, alejandra.lopez@ccscontactcenter.com, isai.hernandez@ccscontactcenter.com ", "***CANCELACION***", MensajeCancelacion)
                            ElseIf DropDownList5.SelectedItem.Text = "Retribucion" Then
                                Alerta.EnviarMail2(GetCorreo(DropDownList10.SelectedItem.Value), "enrique.escalante@alsea.com.mx,roni.cirilo@alsea.com.mx, raul.ayala@ccscontactcenter.com, nancy.souberbielle@ccscontactcenter.com, alejandra.lopez@ccscontactcenter.com, isai.hernandez@ccscontactcenter.com ", "***RETRIBUCION***", MensajeRetribucion)
                            End If
                        Else
                ...
                Else
                    ...
                    If DropDownList5.Visible = True Then
                        ...
                        If DropDownList6.SelectedItem.Text = "Inocuidad " Then
                            LD = GetCorreo(DropDownList14.SelectedItem.Value)
                        ...
                        ElseIf (DropDownList5.SelectedItem.Text = "Cancelacion" Or DropDownList5.SelectedItem.Text = "Retribucion") And HiddenField5.Value = 1 And HiddenField7.Value = 1 Then
                            Cancelar_Pedido()
                            If DropDownList5.SelectedItem.Text = "Cancelacion" Then
                                Alerta.EnviarMail2(GetCorreo(DropDownList10.SelectedItem.Value), "raul.ayala@ccscontactcenter.com, nancy.souberbielle@ccscontactcenter.com, alejandra.lopez@ccscontactcenter.com", "***CANCELACION***", MensajeCancelacion)
                            ElseIf DropDownList5.SelectedItem.Text = "Retribucion" Then
                                Alerta.EnviarMail2(GetCorreo(DropDownList10.SelectedItem.Value), " raul.ayala@ccscontactcenter.com, nancy.souberbielle@ccscontactcenter.com, alejandra.lopez@ccscontactcenter.com ", "***RETRIBUCION***", MensajeRetribucion)
                            End If
                            ClientScript.RegisterStartupScript(GetType(Page), "cualquiercosa", "alv();", True)
                        Else
